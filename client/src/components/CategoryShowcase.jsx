  ...
  return (
    <Section className="py-16 bg-ivory">
      ...
      <motion.div
        ...
        className="sale-banner mb-6 lg:mb-8"
        onClick={() => navigate(salesCategory.route)}
      >
        <h1>{salesCategory.title}</h1>
        {Array.isArray(salesCategory.description) ? (
          <div>
            {salesCategory.description.map((line, idx) => (
              <p key={idx}>{line}</p>
            ))}
          </div>
        ) : (
          <p>{salesCategory.description}</p>
        )}
        <button
